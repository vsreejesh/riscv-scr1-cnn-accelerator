...
//cnn
//static void cmd_cnn_test(void *arg); // <-- ADD THIS LINE
// ...

static void cmd_cnn_test(void *arg);

static void cmd_cnn_test(void *arg)
{
    uart_puts("CNN Accelerator\n");
    uart_puts("Loading image to RAM @ 0x");
    uart_puthex32(PLF_CNN_IMG_BUF_ADDR);
    uart_puts(" (" STRINGIFY(PLF_CNN_IMAGE_SIZE_BYTES) " bytes)...\n");

    int ret = xmodem_receive((uint8_t*)PLF_CNN_IMG_BUF_ADDR, PLF_CNN_IMAGE_SIZE_BYTES);
    if (ret < 0) {
        uart_puts("\nXMODEM load failed: ");
        uart_putdec(-ret);
        uart_puts("\n");
    ...
    uart_puts("\nLoad OK. Copying image to CNN BRAM...\n");

    volatile uint32_t* image_buf = (volatile uint32_t*)PLF_CNN_IMG_BUF_ADDR;
    volatile uint32_t* cnn_bram = PLF_CNN_BRAM_BASE;
    for (int i = 0; i < PLF_CNN_IMAGE_SIZE_WORDS; i++)
        cnn_bram[i] = image_buf[i];

    uart_puts("Starting inference...\n");
    PLF_CNN_CTRL_REG = 0x1;
    while ((PLF_CNN_STATUS_REG & PLF_CNN_STATUS_DONE_BIT) == 0);

    uint32_t status = PLF_CNN_STATUS_REG;
    uint32_t prediction = status & PLF_CNN_STATUS_RESULT_MSK;
    uart_puts("Inference complete. Predicted Digit: ");
    uart_putc((prediction & 0xF) + '0');
    uart_puts("\n");
}
}
static const struct scbl_cmd scbl_commands[] = {
	...
	{'t', "test cnn accelerator", cmd_cnn_test, 0}, // <-- ADD THIS LINE
...
void usage(void)
{
    ...
    uart_puts("t: test cnn accelerator\n"); // <-- ADD THIS LINE
}
